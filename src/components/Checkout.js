...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      enqueueSnackbar('New address have been added successfully', { variant: "success" });
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      enqueueSnackbar('Address has been delete successfully', { variant: "success" });
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if(validateRequest(items, addresses)){
      try {
        const addressId = addresses.all.filter(item => item.address === addresses.selected)[0]._id;
        await axios.post(
        ...
        if (e.response) {
          ...
          enqueueSnackbar(
            "Something went wrong. Failed to make your order.",
            {
              variant: "error",
            }
          );
              ...
              {addresses.all.length ? (
                addresses.all.map((item) => (
                  <Box
                    ...
                    key={item._id}
                    className={addresses.selected === item.address ? 'selected' : 'not-selected'}
                    onClick={()=>setAddresses({...addresses,selected : item.address })}
                  >
